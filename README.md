...
A local Chrome extension for practice assignments. It reads selectable question text, sends a prepared prompt to the ChatGPT website (no ChatGPT API), shows ChatGPT's latest response in the popup, and can type reviewed text into the assignment editor when clipboard paste is blocked. It does not submit answers.
...
6. Reload the practice assignment page and any open ChatGPT tab.
...
6. Click **Ask ChatGPT & show result**. The extension reuses an existing ChatGPT tab when possible, fills the ChatGPT input, and clicks Send in the background.
7. Review ChatGPT's response in **Latest ChatGPT response**. Put the reviewed answer/code into the extension's **Reviewed answer/code** box.
...
- ChatGPT login, CAPTCHA, and submission are left to the user.
